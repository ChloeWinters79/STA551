...
# Introduction
  ...
  This assignment uses synthetic breast cancer data in an effort to explore patterns in tumor characteristics and help evaluate methods and models that may support tumor diagnostic decisions. In this assignment will start with PCA to account for high correlation between tumor measurements. Then the natural grouping will be analyzed with k-means and clustering. Finally the LOF method will be used in determining outliers paired with binary classification to see how LOF impacts the predicition accuracy of the models. 
...
# Results & Conclusions
  ...
  This project showcases a strong distinction between the benign and malignant tumors in this data set. The PCA analysis that was utilized did a good job of reducing the dimensionality in the data set. The results from the k-means and clustering showcased clear grouping in the data corresponding to tumor outcome status, meaning these numerical measurement indicating a natural separation between the tumor types. 
  When using the LOF methodology it unfortunately was not as effective for this data. While LOF is an incredibly useful tool, it is only useful in certain scenarios with certain data sets and this data set was just not meant to benefit from LOF. The malignant tumors do not appear as outliers in the data set causing the LOFs to have low AUCs and poor sensitivities. The supervised models, being logistic and random forest were very strong. With the random forest being slightly stronger and being a great recommendation for a final model in determining whether or not a tumor is benign or malignant. Especially with a sensitivity of 1, this means patients with malignant tumors would never be misdiagnosed which is incredibly beneficial and life saving. Overall, despite the unfortunate results of LOF not being beneficial, the rest of results provided great results for predicting the whether or not a tumor is beniging or malignant, which is the true goal of this project. 
